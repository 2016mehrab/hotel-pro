...
const SalesChart = ({ bookings, numDays }) => {
  ...
  const data = allDates.map(date => {
    return {
      ...
      //a single day can have multiple bookings
  ...
  return (
    <StyledSalesChart>
      <Heading as={"h2"} >Sales from {format(allDates[0], 'MMM dd yyyy')} &mdash; {format(allDates[allDates.length - 1], 'MMM dd yyyy')}</Heading>
